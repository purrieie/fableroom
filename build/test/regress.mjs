...
for(const [w,h] of [[320,780],[390,844],[430,932],[768,1024],[1280,860]]){
  ...
  const gone=await pg.evaluate(()=>['#arBtn','#grainBtn','#detailsBtn','#hsCard','#xrOverlay','#hotspots']
      .filter(s=>document.querySelector(s)));
  const spin=await pg.evaluate(()=>{const t=document.querySelector('#spinTog'),hero=document.querySelector('#hero');
  ...
  // Lustre
  let lustre='not offered';
  try{
    await pg.waitForFunction(()=>{const x=document.querySelector('#hqBtn');return x&&!x.hidden&&!x.classList.contains('is-loading');},{timeout:400000,polling:400});
    await pg.evaluate(()=>document.querySelector('#hqBtn').click());
    await pg.evaluate(()=>new Promise(r=>setTimeout(r,2500)));
    const on=await pg.evaluate(()=>document.querySelector('#hqBtn').classList.contains('on'));
    await pg.evaluate(()=>document.querySelector('#hqBtn').click());
    await pg.evaluate(()=>new Promise(r=>setTimeout(r,1200)));
    const off=await pg.evaluate(()=>document.querySelector('#hqBtn').classList.contains('on'));
    lustre=`toggles on=${on} off=${!off}`;
  }catch(e){ lustre='TIMEOUT'; }

  // other controls still fine
  const ctl=await pg.evaluate(async()=>{
    const out={};
    for(const id of ['studioBtn','dimBtn','scaleBtn']){
  ...
  console.log(`      lustre=${lustre} controls=${JSON.stringify(ctl)} errors=${errs.length?JSON.stringify(errs.slice(0,3)):'none'}`);
  await pg.close();
}
await b.close();
